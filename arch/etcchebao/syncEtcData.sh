...
today=`date +'%Y-%m-%d'`
if [ ! -s "etc-sync.conf" ];then echo "etc-sync.conf不存在"; exit 2; fi
...
# 2. 获取黑名单下载url, 并下载到目录
download() {
    if [ -f "$data_download_path/$today/$1" ]; then echo "$1已存在"
    else
...
download $download_filename $download_source
